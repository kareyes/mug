<script lang="ts">
	...
	const variants: ("default" | "outline" | "underline")[] = [
		"default",
		"outline",
		"underline",
	];
			...
			<PreviewContainer>
				<div class="space-y-4">
					<InputOTP
						...
						error={errorCallbackValue.length > 0 &&
							errorCallbackValue.length < 6}
						onError={handleErrorChange}
					/>
					<p class="text-sm text-muted-foreground">
						Error callback triggered: <span
							class={errorCallbackTriggered
								? "text-destructive"
								: "text-green-600"}
							>{errorCallbackTriggered ? "Yes" : "No"}</span
						>
					</p>
