<script lang="ts">
	import {
		...
		sizesDocs,
		orientationsDocs,
		advancedFeaturesDocs,
		practicalExamplesDocs,
		errorStatesDocs,
		withFieldDocs,
		formIntegrationDocs,
		disabledStateDocs,
		allVariantsSizesDocs,
	} from "$lib/code-blocks/number-spinner";
	import {
		NumberSpinner,
		Field,
		Badge,
	...
	const totalCartItems = $derived(
		(cartQuantity1 ?? 0) + (cartQuantity2 ?? 0) + (cartQuantity3 ?? 0),
	);
	const formValid = $derived(
		formQuantity > 0 && formPrice > 0 && formAge !== null && formAge > 0,
	);
...
<div class="container mx-auto p-6 max-w-6xl">
	...
	<div class="space-y-12">
				...
				<div class="grid gap-8 @md:grid-cols-2">
					<div class="space-y-2">
						...
						<p class="text-sm text-muted-foreground">
							Current value: <Badge variant="secondary"
								>{basicValue}</Badge
							>
						</p>
					</div>
					...
					<div class="space-y-2">
						...
						/>
						<p class="text-sm text-muted-foreground">
							Quantity: <Badge variant="outline"
								>{quantityValue}</Badge
							>
						</p>
					</div>
			...
			<PreviewContainer>
				<div class="grid gap-8 @md:grid-cols-2">
					<div class="space-y-2">
						...
						/>
						<p class="text-sm text-muted-foreground">
							Price: <Badge variant="secondary"
								>${priceValue.toFixed(2)}</Badge
							>
						</p>
					</div>
					...
					<div class="space-y-2">
						...
						/>
						<p class="text-sm text-muted-foreground">
							Temperature: <Badge variant="outline"
								>{temperatureValue}°C</Badge
							>
						</p>
					</div>
						...
						<div class="space-y-3 rounded-lg border p-4">
							...
							<div class="border-t pt-3">
								<p class="text-sm font-medium">
									Total Items: <Badge variant="secondary"
										>{totalCartItems}</Badge
									>
								</p>
							</div>
						</div>
		...
		<Card variant="ghost" class="p-6 bg-background">
			...
			<p class="text-sm text-muted-foreground mb-4">
				Using the Field component for labels, descriptions, and error
				handling.
			</p>
			<PreviewContainer>
			...
			<PreviewContainer>
				...
				>
					...
					<div class="flex items-center justify-between pt-4">
						<p class="text-sm text-muted-foreground">
							Total: <Badge variant="secondary"
								>${(
									(formPrice ?? 0) * (formQuantity ?? 0)
								).toFixed(2)}</Badge
							>
						</p>
